    ...
    with open(outfile, mode) as f:
        ...
        for i in np.arange(samples.shape[0]):
            ...
            out_string = ",".join(
                [
                    str(x)
                    for x in ["FAIR", "FAIR_{}".format(i + 1), "FACTS", *samples[i, :]]
                ]
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the preprocess stage for the emulandice module.",
    ...
    parser.add_argument(
        "--baseyear",
        help="Year to which projections should be referenced",
        type=int,
        default=2005,
    )
